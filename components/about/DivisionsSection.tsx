...
const MemberCard = ({
  ...
  const accentClass =
    DIVISION_ACCENTS[divisionName] || "border-gray-500 text-gray-400";
          ...
          <Image
            src={
              typeof member.image === "string"
                ? member.image
                : urlFor(member.image).width(400).height(600).url()
            }
            alt={member.name}
            fill
  ...
  return (
    <div className="bg-deep-navy pb-20">
      {divisions.map((division, index) => {
        const divisionMembers = Array.isArray(division.members)
          ? division.members
          : [];
        const subdivisions = Array.isArray(division.subdivisions)
          ? division.subdivisions
          : [];

        const accentColor = (
          DIVISION_ACCENTS[division.name] || "text-blue-400"
        ).split(" ")[1];
          ...
          >
            <div className="container px-4 md:px-6">
              ...
              <div className="mb-10">
                ...
                >
                  <h2
                    className={`text-3xl md:text-4xl font-bold mb-4 ${accentColor}`}
                  >
                    {division.name}
                  </h2>
                  <p className="text-slate-400 text-lg mb-6">
                    {division.description ||
                      `The ${division.name} division is an integral part of IEEE SB IPB, driving innovation and excellence in its specific domain.`}
                  </p>
                  ...
                  {subdivisions.length > 0 && (
                    <div className="flex flex-wrap gap-3 mb-8">
                      {subdivisions.map((sub) => (
                        <span
                          key={sub}
                          className="px-3 py-1 rounded-full border border-white/10 bg-white/5 text-sm text-slate-300"
                        >
                          {sub}
                        </span>
                      ))}
