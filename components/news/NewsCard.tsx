  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isFeatured ? "grid md:grid-cols-2 gap-8 p-0" : "flex flex-col h-full",
      )}
    >
      {/* Image Container */}
      <div
        className={cn(
          ...
          isFeatured ? "h-full min-h-[300px]" : "aspect-video",
        )}
      >
        {post.coverImage ? (
          <Image
      ...
      <div
        className={cn(
          "flex flex-col",
          isFeatured ? "p-8 md:p-12 justify-center" : "p-6 flex-grow",
        )}
      >
        ...
        <div className={cn("mt-auto", isFeatured ? "pt-4" : "")}>
          <Button
            variant="link"
            className="p-0 h-auto text-blue-600 font-semibold group/btn"
          >
            <Link
              href={`/news/${post.slug.current}`}
